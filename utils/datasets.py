...
def random_perspective(img, targets=(), degrees=10, translate=.1, scale=.1, shear=10, perspective=0.0, border=(0, 0)):
    ...
    if (border[0] != 0) or (border[1] != 0) or (M != np.eye(3)).any():  # image changed
        if perspective:
            img = cv2.warpPerspective(img, M, dsize=(width, height), borderValue=(0, 0, 0))#for double color
            #img = cv2.warpPerspective(img, M, dsize=(width, height), borderValue=(8, 0, 0))#for single color
        else:  # affine
            img = cv2.warpAffine(img, M[:2], dsize=(width, height), borderValue=(0, 0, 0))#for double color
            #img = cv2.warpPerspective(img, M, dsize=(width, height), borderValue=(8, 0, 0))#for single color
